...
interface Props {
	image?: boolean // 可选参数，用于标识是否显示头像图片
}
defineProps<Props>()

const userStore = useUserStore()

const avatar = computed(() => userStore.userInfo.avatar) // 通过 computed 获取用户头像信息

</script>

<template>
	<template v-if="image"> <!-- 根据传入的参数判断是否需要显示头像图片 -->
		<NAvatar v-if="isString(avatar) && avatar.length > 0" :src="avatar" :fallback-src="defaultAvatar" /> <!-- 根据头像信息是否为字符串以及是否存在，显示用户头像或默认头像 -->
		<NAvatar v-else round :src="defaultAvatar" />
	</template>
	<span v-else class="text-[28px] dark:text-white"> <!-- 如果不需要显示头像图片，显示一个默认的头像图标 -->
